...
const SideBarAllSection = () => {
  ...
  return (
    <div
      className="max-w-sm w-80 bg-white p-8 absolute h-full right-0 bottom-0 top-0"
    ...
    >
      ...
      <ul className="list-disc">
        {allSections?.map((item) => {
          return (
            <li key={item.title} className="text-blue-500 cursor-pointer hover:decoration-dashed">
              {item.title}
            </li>
          );
        })}
